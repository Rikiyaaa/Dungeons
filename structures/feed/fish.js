...
const fish_feed = async (client, interaction, msg, item) => {
    ...
    const inv = await GInv.findOne({ guild: interaction.guild.id, user: interaction.user.id });
        ...
        // if not have fish return msg
        if(object.length === 0) {
            const embed = new EmbedBuilder()
                .setColor(client.color)
                .setDescription("You don't have any fish.")
        ...
        const embed = new EmbedBuilder()
            .setColor(client.color)
            .setDescription("*Please Select a fish*")
    
        const select = new ActionRowBuilder()
            .addComponents([
                new StringSelectMenuBuilder()
                    .setCustomId("fishselect")
                    .setPlaceholder("Select a fish to feed.")
                    .setMaxValues(1)
                    .setMinValues(1)
