...
//go_green_forest_plains

const selectGreen_forest_plains = async (client, interaction, msg ) => {
    ...
    .addComponents(
        new ButtonBuilder()
            .setCustomId("com_green_forest_plains")
            .setLabel("Complete")
            .setStyle(ButtonStyle.Success),
        new ButtonBuilder()
            .setCustomId("can_green_forest_plains")
            .setLabel("Cancel")
            .setStyle(ButtonStyle.Secondary)
        )

        const back_maploc_plains_main_id_b = new ActionRowBuilder()
    .addComponents(
        new ButtonBuilder() 
            .setCustomId("back_maploc_main_id_plains")
            .setLabel("Back")   
    ...
    collector.on('collect', async (menu) => {
        if(menu.isButton()) {
            if(menu.customId === "com_green_forest_plains") {
                ...
                await delay(5000);
                    const embed = new EmbedBuilder()
                    .setDescription(`คุณได้เข้าสู่ **Green Forest plains**`)
                    .setColor(client.color)


                    profile.location = "ป่าลึกสีเขียว";
                    profile.save();

                    await menu.editReply({ embeds: [embed], components: [back_maploc_plains_main_id_b], files: [] });
            
            } else if(menu.customId === "can_green_forest_plains") {
                await menu.deferUpdate();
                await menu.deleteReply();
            } else if(menu.customId === "back_maploc_main_id_plains") {
                await menu.deferUpdate();
                await menu.deleteReply();
            } 
        } 
...
module.exports = { selectGreen_forest_plains };