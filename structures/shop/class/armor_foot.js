...
const { selectArmor_foot_human } = require("./select_class/human/selectArmor_foot_human.js");
const { selectArmor_foot_sword } = require("./select_class/sword/selectArmor_foot_sword.js");
const { selectArmor_foot_warrior } = require("./select_class/warrior/selectArmor_foot_warrior.js");
const { selectArmor_foot_magic } = require("./select_class/magic/selectArmor_foot_magic.js");

const shopArmorFoot = async (client, interaction, msg, item) => {
    ...
    collector.on('collect', async (menu) => {
        if (menu.isButton()) {
            ...
            selectArmor_foot_human(client, interaction, msg, item);
            collector.stop();
            } else if (menu.customId === "sword_id") {
                await menu.deferUpdate();

            selectArmor_foot_sword(client, interaction, msg, item);
            collector.stop();
            } else if (menu.customId === "warrior_id") {
                await menu.deferUpdate();

            selectArmor_foot_warrior(client, interaction, msg, item);
            collector.stop();
            } else if (menu.customId === "magic_id") {
                await menu.deferUpdate();

            selectArmor_foot_magic(client, interaction, msg, item);
            collector.stop();
            }
        }
    });
