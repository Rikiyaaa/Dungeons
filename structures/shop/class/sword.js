...
const { selectSword_human } = require("./select_class/human/selectSword_human.js");
const { selectSword_sword } = require("./select_class/sword/selectSword_sword.js");
const { selectSword_warrior } = require("./select_class/warrior/selectSword_warrior.js");
const { selectSword_magic } = require("./select_class/magic/selectSword_magic.js");

const shopSword = async (client, interaction, msg, item) => {
    ...
    collector.on('collect', async (menu) => {
        if (menu.isButton()) {
            ...
            selectSword_human(client, interaction, msg, item);
            collector.stop();
            } else if (menu.customId === "sword_id") {
                await menu.deferUpdate();

            selectSword_sword(client, interaction, msg, item);
            collector.stop();
            } else if (menu.customId === "warrior_id") {
                await menu.deferUpdate();

            selectSword_warrior(client, interaction, msg, item);
            collector.stop();
            } else if (menu.customId === "magic_id") {
                await menu.deferUpdate();

            selectSword_magic(client, interaction, msg, item);
            collector.stop();
            } 
        }
    });
