...
const { selectArmor_leg_human } = require("./select_class/human/selectArmor_leg_human.js");
const { selectArmor_leg_sword } = require("./select_class/sword/selectArmor_leg_sword.js");
const { selectArmor_leg_warrior } = require("./select_class/warrior/selectArmor_leg_warrior.js");
const { selectArmor_leg_magic } = require("./select_class/magic/selectArmor_leg_magic.js");

const shopArmorLeg = async (client, interaction, msg, item) => {
    ...
    collector.on('collect', async (menu) => {
        if (menu.isButton()) {
            ...
            selectArmor_leg_human(client, interaction, msg, item);
            collector.stop();
            } else if (menu.customId === "sword_id") {
                await menu.deferUpdate();

            selectArmor_leg_sword(client, interaction, msg, item);
            collector.stop();
            } else if (menu.customId === "warrior_id") {
                await menu.deferUpdate();

            selectArmor_leg_warrior(client, interaction, msg, item);
            collector.stop();
            } else if (menu.customId === "magic_id") {
                await menu.deferUpdate();

            selectArmor_leg_magic(client, interaction, msg, item);
            collector.stop();
            }
        }
    });
