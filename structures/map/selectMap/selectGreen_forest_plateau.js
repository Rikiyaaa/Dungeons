...
const selectGreen_forest_plateau = async (client, interaction, msg ) => {
    ...
    .addComponents(
        new ButtonBuilder()
            .setCustomId("com_green_forest_plateau")
            .setLabel("Complete")
            .setStyle(ButtonStyle.Success),
        new ButtonBuilder()
            .setCustomId("can_green_forest_plateau")
            .setLabel("Cancel")
            .setStyle(ButtonStyle.Secondary)
        )

        const back_maploc_plateau_main_id_b = new ActionRowBuilder()
    .addComponents(
        new ButtonBuilder() 
            .setCustomId("back_maploc_main_id_plateau")
            .setLabel("Back")   
    ...
    collector.on('collect', async (menu) => {
        if(menu.isButton()) {
            if(menu.customId === "com_green_forest_plateau") {
                ...
                await delay(5000);
                    const embed = new EmbedBuilder()
                    .setDescription(`คุณได้เข้าสู่ **Green Forest plateau**`)
                    .setColor(client.color)

                    profile.location = "ที่ราบสีเขียว";
                    profile.save();

                    await menu.editReply({ embeds: [embed], components: [back_maploc_plateau_main_id_b], files: [] });
            
            } else if(menu.customId === "can_green_forest_plateau") {
                await menu.deferUpdate();
                await menu.deleteReply();
            } else if(menu.customId === "back_maploc_main_id_plateau") {
                await menu.deferUpdate();
                await menu.deleteReply();
            } 
        } 
...
module.exports = { selectGreen_forest_plateau };