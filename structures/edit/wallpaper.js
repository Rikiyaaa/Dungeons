...
const editWallL = async function (client, interaction,  item, type, id, side, pendingEditHouseCommands) {
    ...
    await interaction.editReply({ embeds: [embed], components: [select], files: [attc] });
    ...
    const hinv = await HInv.findOne({ guild: interaction.guild.id, user: home.user });
        ...
        if(menu.isStringSelectMenu()) {
            ...
            if(menu.customId === "wallplace_l") {
                ...
                if (directory === "place_lone") {
                    /// checking position
                    const check = hinv.wall_left.find(x => x.id === id);
                    if(check.area === 2) {
        ...
        if(menu.isStringSelectMenu()) {
            ...
            if(menu.customId === "wallplace_r") {
                ...
                if (directory === "place_rone") {
                    /// checking position
                    const check = hinv.wall_right.find(x => x.id === id);
                    if(check.area === 2) {
