...
const { selectArmor_head_human } = require("./select_class/human/selectArmor_head_human.js");
const { selectArmor_head_sword } = require("./select_class/sword/selectArmor_head_sword.js");
const { selectArmor_head_warrior } = require("./select_class/warrior/selectArmor_head_warrior.js");
const { selectArmor_head_magic } = require("./select_class/magic/selectArmor_head_magic.js");

const shopArmorHead = async (client, interaction, msg, item) => {
    ...
    collector.on('collect', async (menu) => {
        if (menu.isButton()) {
            ...
            selectArmor_head_human(client, interaction, msg, item);
            collector.stop();
            } else if (menu.customId === "sword_id") {
                await menu.deferUpdate();

            selectArmor_head_sword(client, interaction, msg, item);
            collector.stop();
            } else if (menu.customId === "warrior_id") {
                await menu.deferUpdate();

            selectArmor_head_warrior(client, interaction, msg, item);
            collector.stop();
            } else if (menu.customId === "magic_id") {
                await menu.deferUpdate();

            selectArmor_head_magic(client, interaction, msg, item);
            collector.stop();
            }
        }
    });
