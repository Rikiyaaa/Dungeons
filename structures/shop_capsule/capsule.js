...
const { capsule } = require("../../settings/capsule_item.js");

const shopCapsule = async (client, interaction, item) => {
    if (!interaction && !interaction.channel) throw new Error('Channel is inaccessible.');

        //this returns the values
        const object = Object.values(capsule);

        const row = new ActionRowBuilder()
        .addComponents([
            new StringSelectMenuBuilder()
                .setCustomId("shop_capsule")
        ...
        collector.on('collect', async (menu) => {
            if(menu.isSelectMenu()) {
                if(menu.customId === "shop_capsule") {
                    await menu.deferUpdate();
                    let [ directory ] = menu.values;

                    const item = capsule.find(x => x.name === directory);
                    ...
                    if (gachainv.item.length > profile.gachainventory) return menu.followUp({ content: "You backpack is max " + profile.inventory });
