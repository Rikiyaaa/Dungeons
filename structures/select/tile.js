...
const selectTile = async (client, interaction, pendingEditHouseCommands) => {
    ...
    const object = value.filter(x => x.type === "tile");

    if(object.length === 0) {
        return interaction.editReply({ content: "You don't have any TILE.", embeds: [], files: [], components: [], ephemeral: true  });
    }

    const embed = new EmbedBuilder()
        .setColor(client.color)
        .setDescription("*Please Select a TILE*")

    const select = new ActionRowBuilder()
        .addComponents([
            new StringSelectMenuBuilder()
                .setCustomId("tileselect")
                .setPlaceholder("Select a TILE to placing.")
                .setMaxValues(1)
                .setMinValues(1)
